...
function identity<T extends ValidTypes, U>(value: T, message: U) {
  ...
  //     result = value + value;                // OK
  // }
  //   esta parte es igual a la de arriba, pero se usa una menor cantidad de codigo
...
let returnNumberNoType = identity(100, "Hello!"); // OK este es igual a la anterior linea de codigo
...
function getInventario<T, U extends keyof T>(inventario: T, producto: U): T[keyof T] {
...
type Inventario = {
  martillo: ProductProperties;
  taladro: ProductProperties;
};
let inventario = {
  martillo: {
    precio: 3,
...
type Product = keyof Inventario;
console.log(getInventario<Inventario, Product>(inventario, "martillo"));
